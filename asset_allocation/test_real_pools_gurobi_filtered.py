#!/usr/bin/env python3
"""
Test Script with Real Pools and GUROBI Solver (Filtered APYs)

This script uses real pools from the database but filters out extreme APY values
to test if GUROBI can solve the problem with more reasonable yields.
"""
...
def test_real_pools_with_gurobi_filtered():
    """Test optimization with real pools (filtered APYs) and GUROBI solver"""
    logger.info("\n" + "="*80)
    logger.info("TESTING WITH REAL POOLS (FILTERED APYS) AND GUROBI SOLVER")
    logger.info("="*80)
    ...
    try:
        ...
        # Filter out extreme APY values (cap at 50% annual)
        original_count = len(pools_df)
        extreme_apy_threshold = 50.0  # 50% annual APY
        pools_df = pools_df[pools_df['forecasted_apy'] <= extreme_apy_threshold].copy()
        filtered_count = len(pools_df)
        
        logger.info(f"Filtered pools: {original_count} → {filtered_count} (removed {original_count - filtered_count} with APY > {extreme_apy_threshold:.0f}%)")
        
        if filtered_count == 0:
            logger.error("No pools remaining after filtering")
            return False
        
        # Show APY range after filtering
        logger.info(f"APY range after filtering: {pools_df['forecasted_apy'].min():.2%} - {pools_df['forecasted_apy'].max():.2%}")
        ...
        # Log summary
        logger.info(f"\nTest Setup Summary:")
        logger.info(f"  - Real pools from database: {len(pools_df)} (filtered)")
        logger.info(f"  - APY range: {pools_df['forecasted_apy'].min():.2%} - {pools_df['forecasted_apy'].max():.2%}")
        ...
        if formatted_results['final_allocations']:
            ...
            for pool_id, pool_apy, pool_total in allocations[:5]:
                logger.info(f"  {pool_id} ({pool_apy:.2%}): ${pool_total:,.2f}")
        ...
        filename = f"real_pools_gurobi_filtered_results_{timestamp}.json"
        
        results_data = {
            'timestamp': datetime.now().isoformat(),
            'test_type': 'real_pools_with_gurobi_filtered',
            ...
            'filter_applied': f"APY <= {extreme_apy_threshold:.0%}"
        }
...
def main():
    """Main test function"""
    try:
        success = test_real_pools_with_gurobi_filtered()
        if success:
            logger.info("\n✓ REAL POOLS (FILTERED) WITH GUROBI TEST COMPLETED")
            logger.info("Successfully tested optimization with:")
            logger.info("  - Real filtered pools from database (with APY filtering)")
            logger.info("  - Reasonable forecasted APYs (<= 50%)")
            ...
            logger.error("\n✗ REAL POOLS (FILTERED) WITH GUROBI TEST FAILED")
            return 1
    except Exception as e:
        logger.error(f"\n✗ REAL POOLS (FILTERED) WITH GUROBI TEST FAILED WITH ERROR: {e}", exc_info=True)
        return 1
