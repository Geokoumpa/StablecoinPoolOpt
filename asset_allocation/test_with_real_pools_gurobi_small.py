#!/usr/bin/env python3
"""
Test Script with Real Pools and GUROBI Solver (Small Dataset)

This script uses a subset of real pools from the database with mock assets
to test if GUROBI can solve a smaller-scale problem.
"""
...
def test_real_pools_with_gurobi_small():
    """Test optimization with a smaller subset of real pools and GUROBI solver"""
    logger.info("\n" + "="*80)
    logger.info("TESTING WITH REAL POOLS (SMALL) AND GUROBI SOLVER")
    logger.info("="*80)
    ...
    try:
        ...
        if len(pools_df) == 0:
            ...
            LIMIT 25  -- Only use top 25 pools for this test
            """
            pools_df = pd.read_sql(fallback_query, engine)
            logger.info(f"✓ Loaded {len(pools_df)} pools using fallback query")
        else:
            # If we have pools, limit to top 25 by APY
            pools_df = pools_df.nlargest(25, 'forecasted_apy')
            logger.info(f"✓ Limited to top 25 pools by APY")
        ...
        # Log summary
        logger.info(f"\nTest Setup Summary:")
        logger.info(f"  - Real pools from database: {len(pools_df)} (small subset)")
        ...
        filename = f"real_pools_gurobi_small_results_{timestamp}.json"
        
        results_data = {
            'timestamp': datetime.now().isoformat(),
            'test_type': 'real_pools_with_gurobi_small',
...
def main():
    """Main test function"""
    try:
        success = test_real_pools_with_gurobi_small()
        if success:
            logger.info("\n✓ REAL POOLS (SMALL) WITH GUROBI TEST COMPLETED")
            logger.info("Successfully tested optimization with:")
            logger.info("  - Subset of real filtered pools from database")
            ...
            logger.error("\n✗ REAL POOLS (SMALL) WITH GUROBI TEST FAILED")
            return 1
    except Exception as e:
        logger.error(f"\n✗ REAL POOLS (SMALL) WITH GUROBI TEST FAILED WITH ERROR: {e}", exc_info=True)
        return 1
